...
public class ImporterPajek implements FileImporter, LongTask {
    ...
    private void importData(LineNumberReader reader) throws Exception {
        ...
        try {
            ...
            Integer num_in_bipartie = null;
            if ( stringTokenizer.hasMoreElements()) {
                num_in_bipartie = Integer.parseInt(stringTokenizer.nextToken());
            }
            ...
            //Read matirx
            readMatrix(curLine, reader, num_in_bipartie);
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        } finally {
    ...
    private String readMatrix(final String curLine, final BufferedReader br,
            final Integer numInBipartie) throws Exception {
        String nextLine = curLine;

        if (null==numInBipartie || !nextLine.toLowerCase().startsWith("*matrix")) {
            return curLine;
        }
        if ( numInBipartie>verticesArray.length || numInBipartie<0) {
            report.logIssue(new Issue(NbBundle.getMessage(ImporterPajek.class, "importerNET_error_dataformat6", numInBipartie, lineReader.getLineNumber()), Issue.Level.CRITICAL));
            return curLine;
        }

        int nodeTo = 0;
        while (br.ready() && nodeTo<(numInBipartie-1)) {
            if (cancel) {
                return nextLine;
            }
            nextLine = br.readLine();
            ...
            final StringTokenizer st = new StringTokenizer(nextLine.trim());
            int nodeFrom = numInBipartie-1;
            while (st.hasMoreElements() && nodeFrom<(verticesArray.length-1)) {
                int weight = Integer.parseInt(st.nextToken());
                if ( weight != 0) {
                    EdgeDraft edge = container.factory().newEdgeDraft();
                    edge.setSource(verticesArray[nodeFrom]);
                    edge.setTarget(verticesArray[nodeTo]);
